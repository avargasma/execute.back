/****** Object:  StoredProcedure [MC].[Conversation_s_Comments]    Script Date: 5/03/2020 11:25:47 a. m. ******/
SET ANSI_NULLS ON
GO
...
-- Description:	Se agrega en la consulta final inner join MC.PublicationDetail para mostrar el campo PDCommentAux5
-- JIRA: EONC-601
-- ============================================= 
...
BEGIN  
 ...
 SET @P1ConversationId = 39001 --@ConversationId 
 SET @P1Row = null -- @Row    
 SET @P1Ascendent = null -- @Ascendent
 SET @P1DaysRange = null -- @DaysRange
 ...
 -- left(cc.[CaseCommentText],5000) as CaseCommentText, 
  ...
  ,cc.[CaseCommentModifiedByUserId]  
  ,cc.[ElementTypeId], et.PChannelType, cc.[ElementId],  
  et.ElementTypePublic, et.ElementTypeOutput, et.ServiceChannelId,dcc.[DCCSubject],  
  sc.SCName, p.PublicationDate, p.[PRetriesQuantity],p.PublicationErrorId, sc.SCInternalCode, cc.ProcessDetailsId,   
  dcc.DCCPermlinkRoot,dcc.DCCOnline,  
  sc.IsTextHtml, cc.ConversationId, cc.GroupAccountId, cc.CaseCommentPublicationTo,  
  uc.UCUserName,uc.UCName, cc.CreationDateLog, cad.AccountDetailDescrip, cc.Agent, pd.PDCommentAux5   
  FROM #tb cc  
  ...
  /*LEFT JOIN mc.CaseCommentPublication ccp ON ccp.CaseCommentId=cc.CaseCommentId    
  LEFT JOIN mc.Publication p ON ccp.PublicationId=p.PublicationId*/) cc  
 ...
 BEGIN    
  ...
  ,cc.[ElementTypeId], et.PChannelType, cc.[ElementId],
  et.ElementTypePublic, et.ElementTypeOutput, et.ServiceChannelId,dcc.[DCCSubject],  
  sc.SCName, p.PublicationDate, p.[PRetriesQuantity], p.PublicationErrorId, sc.SCInternalCode, cc.ProcessDetailsId, dcc.DCCPermlinkRoot,  
  dcc.DCCOnline,sc.IsTextHtml, cc.ConversationId, cc.GroupAccountId,   
  @PCurrentDate AS ServerDate, cc.CaseCommentPublicationTo, uc.UCUserName, uc.UCName, cc.CreationDateLog,  
  cad.AccountDetailDescrip,cc.Agent, pd.PDCommentAux5   
  FROM #tb cc  
  INNER JOIN MC.UserChannel uc On uc.UserChannelId = cc.UserChannelId  
  INNER JOIN mc.ElementType et ON cc.ElementTypeId = et.ElementTypeId    
  INNER JOIN config.ServiceChannel sc ON sc.ServiceChannelId=et.ServiceChannelId    
